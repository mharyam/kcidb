...
class DeploymentEmailTestCase(unittest.TestCase):
    ...
    def test_email_generated(self):
        ...
        # Try to pull the four notification messages we're expecting
        msgs = email_subscriber.pull(4, 1800)
        # Check we got exactly four
        self.assertEqual(len(msgs), 4)
        # Check we get one correct message per object type
        obj_types = {"revision", "checkout", "build", "test"}
        for msg in msgs:
            email_subscriber.ack(msg[0])
            email = msg[1]
        ...
        msgs = email_subscriber.pull(1, 5)
